...
BDR.game = (function() {
  ...
  function buildPhysics() {
    ...
    // Hazard collision bodies: manual effects remain authoritative, but these
    // physical bodies prevent balls/projectiles from visually clipping through
    // solid obstacles such as bumpers and moving pillars.
    for (const hz of (mazeData.hazardMeshes || [])) {
      if (hz.type === 'bumper') {
        const radius = cfg.cellSize * 0.25;
        const shape = new CANNON.Cylinder(radius, radius, 1.25, 24);
        const body = new CANNON.Body({ mass: 0, material: wallMat });
        body.addShape(shape);
        body.position.set(hz.x * cfg.cellSize, 0.62, hz.y * cfg.cellSize);
        physWorld.addBody(body);
        hz.body = body;
        hz.radius = radius;
      } else if (hz.type === 'mover') {
        const shape = new CANNON.Box(new CANNON.Vec3(0.52, wallH * 0.42, 0.52));
        const body = new CANNON.Body({ mass: 0, material: wallMat });
        body.addShape(shape);
        body.position.set(hz.baseX, wallH * 0.42, hz.baseZ);
        physWorld.addBody(body);
        hz.body = body;
      } else if (hz.type === 'mine') {
        hz.triggerRadius = cfg.cellSize * 0.48;
        hz.explosionRadius = cfg.cellSize * 1.45;
      }
    }
  ...
  function applyHazards(dt) {
    ...
    function detonateMine(hz, cx, cz, sourcePlayer) {
      hz.cooldownUntil = now + 3500;
      if (hz.mesh) hz.mesh.visible = false;
      if (hz.warn) hz.warn.visible = false;
      setTimeout(() => {
        try {
          if (hz.mesh) hz.mesh.visible = true;
          if (hz.warn) hz.warn.visible = true;
        } catch(e) {}
      }, 2800);
      if (BDR.items) BDR.items.spawnHitFlash(scene, { x: cx, y: 0.5, z: cz }, 0xff4757);
      const radius = hz.explosionRadius || cs * 1.45;
      for (const q of players) {
        ...
        if (d < radius) {
          ...
          const power = 22 * falloff + 4;
          q.body.velocity.x += (qx / n) * power;
          q.body.velocity.z += (qz / n) * power;
          q.body.velocity.y += 4.5 + 2.5 * falloff;
          q.hp = Math.max(0, q.hp - Math.round(22 * falloff));
        }
      }
      if (sourcePlayer && sourcePlayer.isMe && BDR.sound) BDR.sound.bump();
    }
    ...
    for (const hz of mazeData.hazardMeshes) {
      if (hz.type === 'spinner') {
        ...
        if (hz.body) {
          hz.body.position.set(hz.mesh.position.x, hz.body.position.y, hz.mesh.position.z);
          hz.body.aabbNeedsUpdate = true;
        }
        // Bounce balls that touch it (also backed by a Cannon body to stop clipping)
        ...
        if (hz.warn) hz.warn.material.opacity = 0.34 + Math.sin(t * 8) * 0.18;
        const cx = hz.x * cs, cz = hz.y * cs;
        if (now < hz.cooldownUntil) continue;
        let detonator = null;
        const triggerRadius = hz.triggerRadius || cs * 0.48;
        for (const p of players) {
          if (p.finished) continue;
          const dx = p.body.position.x - cx;
          const dz = p.body.position.z - cz;
          if (dx*dx + dz*dz < triggerRadius*triggerRadius && p.body.position.y < 1.8) { detonator = p; break; }
        }
        // Shots can also detonate mines, giving players a tactical way to clear routes.
        if (!detonator) {
          for (const pj of projectiles) {
            const dx = pj.body.position.x - cx;
            const dz = pj.body.position.z - cz;
            if (dx*dx + dz*dz < triggerRadius*triggerRadius && pj.body.position.y < 2.2) { pj.hit = true; detonator = players.find(pp => pp.id === pj.ownerId) || null; break; }
          }
        }
        if (detonator) detonateMine(hz, cx, cz, detonator);
      } else if (hz.type === 'bumper') {
        if (hz.mesh) hz.mesh.rotation.y = t * 1.7;
        const cx = hz.x * cs, cz = hz.y * cs;
        const hitRadius = (hz.radius || cs * 0.25) + cfg.ballRadius + 0.08;
        for (const p of players) {
          if (p.finished) continue;
          const dx = p.body.position.x - cx;
          const dz = p.body.position.z - cz;
          const d = Math.hypot(dx, dz);
          if (d < hitRadius && p.body.position.y < 1.9) {
            const n = d || 1;
            const nx = dx / n, nz = dz / n;
            const cooldownReady = now - (p._lastBumperAt || 0) > 220;
            if (cooldownReady) {
              p.body.velocity.x = nx * Math.max(13, Math.abs(p.body.velocity.x) + 8);
              p.body.velocity.z = nz * Math.max(13, Math.abs(p.body.velocity.z) + 8);
              p.body.velocity.y += 2.8;
              p._lastBumperAt = now;
              if (BDR.items) BDR.items.spawnHitFlash(scene, { x: cx + nx * hitRadius, y: 0.8, z: cz + nz * hitRadius }, 0xa855f7);
              if (p.isMe && BDR.sound) BDR.sound.bump();
            }
            p.body.position.x = cx + nx * hitRadius;
            p.body.position.z = cz + nz * hitRadius;
          }
        }
      } else if (hz.type === 'tp') {
        // Visual spin
